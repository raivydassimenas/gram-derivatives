...
**Karatsuba–Korolev representation** — `S(t)` decomposes as
```
S(t) = φ(t) − (1/π) δ(t) + N_step(t)
```
where this identity holds on each open interval between consecutive ordinates of zeros of ζ. Use `S_eq_φ_sub_δ_add_N` as the axiom name (already present in `Theorem1.lean`).
...
**Analytic properties of `N_step`** — `N_step : ℝ → ℝ` is locally constant on each open interval between consecutive ζ-zero ordinates, so its n-th iterated derivative (`n ≥ 1`) vanishes there. Axiom name: `N_step_iteratedDeriv_eq_zero` (already in `Theorem1.lean`).
