...
- **`N_step` is a concrete piecewise-constant function**: it is *defined* as the floor function `N_step t = ⌊t⌋`, with `JumpSet = Set.range (Int.cast : ℤ → ℝ)` its discontinuity set. It stands in for the zero-counting step function `N(γ+0)`; the proof uses only that it is locally constant off `JumpSet` — `contDiffAt_N_step` and `N_step_iteratedDeriv_eq_zero` are **theorems** proved from that local constancy (formerly axioms). Any piecewise-constant function would serve; do not add ζ-specific properties.
...
- **`N_step`** morally plays the role of `N(γ+0)`, the right-continuous Riemann ζ zero-counting function: the number of nontrivial zeros `ρ = β + iγ` with `0 < γ ≤ t`, `0 ≤ β ≤ 1` — integer-valued and piecewise constant on `(0, ∞)`, with jumps exactly at the ordinates of ζ-zeros. (Formally, `N_step` is now *defined* in `Theorem1.lean` as the floor function, a stand-in with the same locally-constant structure; `N_step_iteratedDeriv_eq_zero` is a theorem about it.)
...
**Analytic properties of `N_step`** — `N_step : ℝ → ℝ` is *defined* in `Theorem1.lean` as the floor function `⌊·⌋`, a concrete piecewise-constant (locally constant) function; the proof only uses local constancy off `JumpSet` (the integers), so it stands in for any step function. Being locally constant, its n-th iterated derivative (`n ≥ 1`) vanishes. Lemma name: `N_step_iteratedDeriv_eq_zero` (a theorem in `Theorem1.lean`, formerly an axiom). When working in `Corollary2.lean`, `N_step` morally plays the role of the right-continuous Riemann ζ zero-counting function `N(γ+0)`, which is integer-valued and jumps only at the ordinates of nontrivial ζ-zeros.
