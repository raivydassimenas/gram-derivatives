...
Remaining axiom (tagged `-- ASSUMPTION`); a statement about the *defined* `gram`. The strict monotonicity of `theta` on `[7, ∞)` that justifies it is now **formal** (`strictMonoOn_theta` in `Corollary2.lean` §7, axiom-free, via the explicit pointwise bound `abs_deriv_δ_le : |δ'(t)| ≤ 1/t²` from `Theorem1.lean` §7a), and `gram_theta` (`Theorem3.lean` §1) shows `gram` is a genuine left inverse; only the inverse-function-theorem step itself remains unformalised:
